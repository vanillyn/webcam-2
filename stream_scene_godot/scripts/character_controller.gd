...
const HEAD_YAW_LIMIT   := 28.0
const HEAD_PITCH_LIMIT := 18.0


const NECK_FACTOR  := 0.32
const SPINE_FACTOR := 0.12
...
func _load_character() -> void:
	...
	if root_h > 5.0:
		...
		print("[CharacterController] Model height %.2f — applying 0.01 scale." % root_h)
	else:
	...
	if _skeleton == null:
		...
		if _head_bone == -1:
			push_warning("[CharacterController] Head bone '%s' not found. Bones: %s"
				% [BONE_HEAD, _list_bones(_skeleton)])
		else:
			print("[CharacterController] Ready. Head bone idx=%d" % _head_bone)
	...
	_model_loaded = true
	print("[CharacterController] Character loaded.")
	_apply_sit_pose()
...
func _apply_sit_pose() -> void:
	...
	for bone_name: String in pose_map:
		...
		var rot_q := Quaternion.from_euler(Vector3(deg_to_rad(rd.x), deg_to_rad(rd.y), deg_to_rad(rd.z)))
		_skeleton.set_bone_pose_rotation(idx, rot_q)
...
func _process(delta: float) -> void:
	...
	_target_head_rot.x += sin(_time * 0.7) * 0.5
	_target_head_rot.z  = sin(_time * 0.45) * 0.35
...
func _spawn_placeholder() -> void:
	...
	add_child(body)
	push_warning("[CharacterController] Using placeholder — no GLB found.")
